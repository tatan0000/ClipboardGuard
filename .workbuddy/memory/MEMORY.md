...
## 开机初始化策略（v5 - 2026-05-05）
- **开机立即注册必定失败**（AMS 未就绪），不再尝试
- **5s 后第一次延迟注册**，**8s 后第二次延迟注册**（兜底）
- **Hook 侧只注册广播接收器，被动等待 App 推送配置**（不主动读文件、不拉起 Service）
- **BootReceiver 启动 ConfigSyncService 前台服务**：`startForegroundService()` → `onCreate` 中 `startForeground()` → 子线程延迟 12s/15s 发广播 → `stopSelf()`
- 前台服务比 `goAsync()` 更可靠（goAsync 最多 10 秒，我们需要 12 秒才能发广播）
- BootReceiver 加 60 秒防抖（USER_PRESENT 等可能多次触发）
- Manifest 需要 `FOREGROUND_SERVICE` + `FOREGROUND_SERVICE_SPECIAL_USE` 权限，service 设 `foregroundServiceType="specialUse"`
- **通知由 ConfigSyncService 显示**（前台通知），服务结束后自动消失

### 推送职责分工
| 触发时机 | 调用方法 | 携带内容 |
|---------|---------|---------|
| 开机自启动 | BootReceiver → ConfigSyncService(12s/15s) → `sendFullConfigBroadcast` | blocklist + 规则 JSON |
...
BootReceiver 收到开机/解锁/时间变化广播（任一触发）：
  └─ startForegroundService(ConfigSyncService)
  └─ ConfigSyncService.onCreate → startForeground（显示通知）
  └─ 12s 后发配置广播 → Hook 侧收到 ✅
  └─ 15s 后发第二次广播（兜底）✅
  └─ 更新通知「剪贴板保护服务已启动」
  └─ stopForeground + stopSelf（服务退出，通知消失）

用户复制时：ensureInitialized() → 确保广播接收器已注册
```
