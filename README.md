...
## TODO
有时间就更新
| 计划编号 | 计划内容 | 难度 | 可行性 | 主要挑战与说明 |
|----------|----------|------|--------|----------------|
| 1 | 完善读取 Hook 逻辑<br>• 配置及时更新<br>• 未开启正则 → 拒绝读取 + Toast<br>• 开启正则且命中 → 弹窗（允许/拒绝/拒绝并清空） | ⭐⭐⭐ 中等 | ✅ 高 | 可复用写入拦截框架；需扩展 `InlineDialogManager` 支持三按钮；缓存更新机制已成熟 |
| 2 | 日志迁移至 LSPosed Manager<br>• 删除 App 设置页的日志相关界面<br>• 日志写入 LSPosed 模块日志 | ⭐⭐ 简单 | ✅ 高 | 改用 `XposedBridge.log` 或 `Log.i` 由 LSPosed 自动收集；移除 UI 简单 |
| 3 | Magisk 模块版<br>• 制作独立 Magisk 模块，不依赖 LSPosed | ⭐⭐⭐⭐ 高 | ⚠️ 中等 | 需用 Zygisk C++ 重写 Hook/弹窗/文件读取；几乎所有 Java 代码不可复用；开发周期长 |
| 4 | 免 Root 版<br>• 无障碍 + Shizuku/ADB 写入广告后清空 | ⭐⭐⭐ 中等 | ✅ 高 | 无障碍可监听剪贴板+获取包名；Shizuku 清空受限需测试；规则引擎可复用；注意无障碍合规 |
